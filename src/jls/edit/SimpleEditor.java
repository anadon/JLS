...
/**
 * Main circuit editing class. Sets up and manages GUI.
 * 
...
public abstract class SimpleEditor extends JPanel {
	...
	protected JLabel editable = new JLabel(" "); // to show if circuit editing
													// is enabled
	private JLabel message = new JLabel(" "); // editing status message display
	private JLabel info = new JLabel(" ", SwingConstants.CENTER); // element
																	// information
																	// display
	private Circuit clipboard; // for cut and paste
	private JPopupMenu importMenu = new JPopupMenu(); // to display importable
														// circuits
	private SimpleEditor me;
	private Stack<Circuit> undos = new Stack<Circuit>();
	private Stack<Circuit> redos = new Stack<Circuit>();
	private int check = JLSInfo.checkPointFreq + 1; // number of changes, used
													// for checkpointing
	 ...
	 * 
	 * @param pane
	 *            The tabbed pane this editor is in.
	 * @param circuit
	 *            The circuit it will edit.
	 * @param name
	 *            The name of the circuit.
	 * @param clipboard
	 *            For cut and paste.
	 */
	public SimpleEditor(JTabbedPane parent, Circuit circuit, String name,
			Circuit clipboard) {
		...
		ew = new EditWindow();
		ew.setPreferredSize(new Dimension(JLSInfo.circuitsize,
				JLSInfo.circuitsize));
		...
		// set up increase circuit size button in lower right corner of scroll
		// pane
	...
	public void increaseSize() {
		...
		Dimension newSize = new Dimension((int) (size.width * 1.1),
				(int) (size.height * 1.1));
	...
	/**
	 * Make the editor able to make changes. Usually turned off during
	 * simulation.
	 * 
	 * @param which
	 *            True to enable editing, false to disable it.
	 */
	...
	private enum State {
		idle, chosen, placing, moving, selecting, selected, option, startwire, drawire
	};

	/**
	 * The window the circuit is displayed and edited in.
	 */
	private class EditWindow extends JPanel implements ActionListener,
			MouseListener, MouseMotionListener {

		// popup menus
		private JPopupMenu optionMenu = new JPopupMenu();
		private JMenuItem probe = new JMenuItem(""); // will get title when
														// added to menu
		private JMenuItem watch = new JMenuItem(""); // will get title when
														// added to menu
		...
		private Set<Element> selected = new HashSet<Element>(); // currently
																// selected
																// elements
		...
		private Set<Element> adds = new HashSet<Element>(); // for adding
															// elements during a
															// connect
		private Set<Element> subs = new HashSet<Element>(); // for removing
															// elements during a
															// connect
		...
		public EditWindow() {
			...
			probe.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P,
					InputEvent.CTRL_MASK));
			watch.setToolTipText("watch activity on this element during simulation");
			watch.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_W,
					InputEvent.CTRL_MASK));
			modify.setToolTipText("view/modify element details");
			modify.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_M,
					InputEvent.CTRL_MASK));
			timing.setToolTipText("change propagation delay or access time");
			timing.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_T,
					InputEvent.CTRL_MASK));
			view.setToolTipText("view current simulated value");
			view.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S,
					InputEvent.CTRL_MASK));
			...
			lock.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L,
					InputEvent.CTRL_MASK));
			...
			connect.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_W,
					InputEvent.CTRL_MASK));
			newMenu.add(connect);
			...
			selAll.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A,
					InputEvent.CTRL_MASK));
			newMenu.add(selAll);
			...
			getInputMap()
					.put(KeyStroke.getKeyStroke(KeyEvent.VK_M,
							InputEvent.CTRL_MASK), "modify");
			...
			getInputMap()
					.put(KeyStroke.getKeyStroke(KeyEvent.VK_T,
							InputEvent.CTRL_MASK), "timing");
			...
			getInputMap()
					.put(KeyStroke.getKeyStroke(KeyEvent.VK_A,
							InputEvent.CTRL_MASK), "select all");
			...
			getInputMap()
					.put(KeyStroke.getKeyStroke(KeyEvent.VK_E,
							InputEvent.CTRL_MASK), "close window");
		...
		public void makeElements() {
			...
			act = new AbstractAction(text, image) {
				public void actionPerformed(ActionEvent event) {
					setup(new StateMachine(circuit),
							event.getSource() instanceof JButton);
				}
			};
		...
		public void mouseMoved(MouseEvent event) {
			...
			if (currentState == State.placing
				...
				if (overlap()) {
					...
					if (!(currentState == State.drawire && prev != null
							&& prev.getX() == wireEnd.getX() && prev.getY() == wireEnd
							.getY())) {
						info.setText(overlapMessage);
						info.setForeground(Color.red);
					}
		...
		/**
		 * See if wire ends can connect. Not possible if one end is not
		 * dangling. Not possible if part of the same wire net. Not possible if
		 * bits don't match. Not possible if both have inputs already.
		 * 
		 * @param end1
		 *            The wire being connected.
		 * @param end2
		 *            The other wire being connected.
		 * 
		...
		/**
		 * Connect exiting wire ends. Assumes ends can be connected (overlap,
		 * one end dangling, one input, bits match, not same net). One wire end
		 * will be deleted, and the remaining one is returned.
		 * 
		 * @param end1
		 *            The wire end being connected.
		 * @param end2
		 *            The other wire end being connected.
		 * 
		...
		/**
		 * Connect a wire end to a wire. Return the end that was connected.
		 * 
		 * @param end
		 *            The wire end.
		 * @param wire
		...
		/**
		 * See if a wire end can attach to a put. Not possible if put is already
		 * attached, or bits don't match, or put is an output and wire end is
		 * already connected to an input.
		 * 
		 * @param end
		 *            The wire end.
		 * @param put
		 *            The put.
		 * 
		 * @return false if put is already attached or bits do not match, true
		 *         otherwise.
		 */
		...
		/**
		 * Attach a wire end to a put. The wire end must be dangling. The put
		 * must not be attached. Bits must match, and the wire net must not
		 * already have an input if this put is an output. Bits will be set in
		 * the net. Has-input will be set in the net if necessary. Tri-state
		 * will be set in the net if necessary.
		 * 
		 * @param end
		 *            The wire end.
		 * @param put
		...
		/**
		 * See if a put can attach to another put. Not possible of they don't
		 * overlap. Not possible if either put is already attached. Not possible
		 * if bits don't match. Not possible if both are outputs or both are
		 * inputs, unless both are tristate outputs.
		 * 
		 * @param put1
		 *            The put.
		 * @param put2
		...
		/**
		 * Attach a put to a put by creating a wire net with two wire ends and a
		 * wire. The puts must overlap. The puts must not be attached. Bits must
		 * match
		 * 
		 * @param p1
		 *            The put.
			...
			for (Element sel : selected) {
				...
				for (Element el : circuit.getElements()) {
					...
					if (sel.intersects(el)) {
						...
						// selected is not a wire end
							...
							for (Put put : sel.getAllPuts()) {
								...
								WireEnd putEnd = put.getWireEnd();
								if (putEnd != null
										&& putEnd.getOnlyWire().getOtherEnd(
												putEnd) == end) {
									ok = true;
									continue;
								}
					...
					// or puts might line up
						...
						// see if wires connected to a wire end dragged onto
						// wire ends
						if (sel instanceof WireEnd) {
							WireEnd end = (WireEnd) sel;
		...
		/**
		 * Connect everything possible. Assumes there is no overlap.
		 */
		public void connect() {
		...
		/**
		 * Remove all elements in the selected set. Abort removal if any
		 * elements in the set are uneditable.
		 */
		private void remove() {
		 ...
		 * Can't be done if there are elements in the "from" circuit that have
		 * the same names as elements in the current circuit.
		 * 
		 * @param from
		 *            The circuit to copy from.
		 * 
			...
			for (Element el : from.getElements()) {
				if (el instanceof JumpEnd) {
					...
					if (!jsnames.contains(name)
							&& circuit.getJumpStart(name) == null) {
						info.setForeground(Color.red);
		...
		/**
		 * Reset highlights of elements in selected set, then clear selected
		 * set.
		 */
		...
		/**
		 * Select all elements currently in the circuit, except attached wire
		 * ends.
		 */
		...
		private void doModify() {
			...
			if (el instanceof SubCircuit) {
				...
				Editor ed = new Editor(tabbedParent, subcirc, sub.getName(),
						clipboard);
		...
		/**
		 * Remove any wire end that has degree 2 and is co-linear (horizontally
		 * or vertically) with opposite ends, or that has degree 1, not attached
		 * and is in the same place as its other end.
		 */
		private void removeCoLinear() {
			...
			for (WireEnd end : ends) {
				...
				if (end.degree() == 1) {
					...
					if (end.isAttached() || !otherEnd.isAttached()
							|| !end.intersects(otherEnd))
						continue;
					wire.remove(circuit);
				}
		...
		/**
		 * Set up new element if editor is enabled. Usually pops up dialog to
		 * enter characteristics. If not cancelled, then adds it to the circuit
		 * and gets ready to place it.
		 * 
		 * @param item
		 *            The element to set up.
		 * @param fromToolBar
		 *            True if toolbar button selected, false if from menu.
		 */
		...
		public void doImport(String name) {
			...
			// make a set of all elements except attached wire ends from
			// subcircuit
		...
		/**
		 * Mark the editted circuit as changed, save a copy of the circuit for
		 * undo, and checkpoint file if it is time.
		 */
		public void markChanged() {
			...
			if (check > JLSInfo.checkPointFreq) {
				...
				String fileName = circ.getDirectory() + "/" + circ.getName()
						+ ".jls~";
		...
		/**
		 * Finish up undo or redo. Used by undo and redo to make a copy of the
		 * circuit.
		 * 
		 * @param temp
		 *            The circuit being copied.
		 */
		...
		/**
		 * Find all jump start elements and add names to the jumpstart list in
		 * this circuit. Do the same for all subcircuits.
		 * 
		 * @param circ
		...
		/**
		 * Find all named elements and add names to the namesUsed list in this
		 * circuit. Do the same for all subcircuits.
		 * 
		 * @param The
		 *            circuit to process.
		 */
