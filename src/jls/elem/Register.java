...
public class Register extends LogicElement {
	...
	public void draw(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			...
			ly = three.getY();
			g.setColor(Color.black);
			g.drawString("Q", lx - fm.stringWidth("Q") - 1,
					ly - h / 2 + fm.getAscent());
			three.draw(g);
			...
			g.drawString("Q", lx - fm.stringWidth("Q") - 1,
					ly - h / 2 + fm.getAscent());
			g.drawLine(lx - fm.stringWidth("Q") - 1, ly - h / 2, lx - 2, ly - h
					/ 2);
			four.draw(g);
		} else if (orientation == JLSInfo.Orientation.LEFT) {
			...
			g.setColor(Color.black);
			g.drawString("Q", lx + 3, ly - h / 2 + fm.getAscent());
			g.drawLine(lx + fm.stringWidth("Q") + 2, ly - h / 2, lx + 2, ly - h
					/ 2);
			four.draw(g);
		} else if (orientation == JLSInfo.Orientation.UP) {
			...
			int h = fm.getAscent() + fm.getDescent();
			g.drawString("D", lx - fm.stringWidth("D") / 2,
					ly - s - h + fm.getAscent());
			one.draw(g);
			...
			case Latch:
				...
				g.drawString("C", lx - fm.stringWidth("C") / 2,
						ly - s - h + fm.getAscent());
				break;
			case PosFF:
				g.drawLine(lx, ly - s, lx, ly);
				...
				g.drawLine(lx + d, ly - s, lx, ly - s - d);
				g.drawString("C", lx - fm.stringWidth("C") / 2, ly - s - d - h
						+ fm.getAscent());
				break;
			case NegFF:
			...
			ly = three.getY();
			g.setColor(Color.black);
			g.drawString("Q", lx - fm.stringWidth("Q") / 2,
					ly + d + fm.getAscent());
			three.draw(g);
			...
			g.drawString("Q", lx - fm.stringWidth("Q") / 2,
					ly + d + fm.getAscent());
			g.drawLine(lx - fm.stringWidth("Q") / 2, ly + d - 1,
					lx + fm.stringWidth("Q") / 2, ly + d - 1);
			four.draw(g);
			...
			g.drawString("Q", lx - fm.stringWidth("Q") / 2,
					ly - h + fm.getAscent());
			g.drawLine(lx - fm.stringWidth("Q") / 2, ly - h,
					lx + fm.stringWidth("Q") / 2, ly - h);
			four.draw(g);
	...
	/**
	 * Set a long instance variable value (during a load). This is here for
	 * compatibility with early version of JLS that saved the initial value as a
	 * long instead of a BigInteger.
	 * 
	 * @param name
	 *            The instance variable name.
	 * @param value
	...
	/**
	 * Tells if a register is capable of rotatating, can only rotate when inputs
	 * or outputs have no attachments.
	 * 
	...
	/**
	 * Tells if a register is capable of flipping, can only flip when inputs or
	...
	private class RegisterEdit extends JDialog implements ActionListener {
		...
		private KeyPad valuePad = new KeyPad(valueField, 16,
				initialValue.longValue(), this);
		 ...
		 * @param creating
		 *            True if creating, false if modifying.
		 */
	...
	public void showCurrentValue(Point where) {
		...
		String value = "0x" + hex + " (" + unsigned + " unsigned, " + signed
				+ " signed)";
