...
/**
 * Superclass of binder/splitter. Contains common info and method.
 * 
...
public abstract class Group extends LogicElement {
	...
	/**
	 * Tells if a Group is capable of flippiing, can only flip when inputs or
	...
	public void save(PrintWriter output) {
		...
		// noncontig is always true for newer files. Without this, JLS assumes
		// false (legacy save)
	...
	protected class GetRanges extends JDialog implements ActionListener {
		...
		private JList<Entry> choose = extracted_choose(); // LeftList?

		private JList<Entry> extracted_choose() {
			return new JList<Entry>(pick);
		}

		private DefaultListModel<Entry> picked = new DefaultListModel<Entry>();
		private JList<Entry> chosen = extracted_chosen();

		private JList<Entry> extracted_chosen() {
			return new JList<Entry>(picked);
		}
		...
		/**
		 * Dummy constructor, for internal use only. Permits use of a kludgy fix
		 * that permits instantiation of an Entry from within a GroupCreate
		 * object. Since Entry is a subclass of GetRanges, which is in turn a
		 * sibling of GroupCreate, Java will not allow GroupCreate to
		 * instantiate an Entry unless it has a reference to an instance of
		 * GetRanges. This protected constructor allows us to do exactly that,
		 * without creating the entire Swing dialog.
		 */
		protected GetRanges() {
		...
		public void actionPerformed(ActionEvent event) {
			if (event.getSource() == add) {
				...
				// add entry to the end of the list (user can resort it
				// themselves)
				...
				if (picked.size() == 0) {
					JOptionPane.showMessageDialog(this, "No groups chosen",
	...
	protected class Entry {
		 ...
		 * 
		 * @param min
		 *            lowest value to store
		 * @param max
		 *            highest value to store
		 */
