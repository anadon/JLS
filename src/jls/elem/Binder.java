...
public class Binder extends Group implements TriProp {
	...
	public void init(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			int ypos = s;
			for (Entry e : ranges) {
				inputs.add(new Input(e.toCircuitString(), this, 0, ypos, e
						.getSize()));
				ypos += s;
			}

			// set up output
			outputs.add(new Output("output", this, width,
					((ranges.size() - 1) / 2 + 1) * s, bits));
			if (loadTriState) {
			...
			int ypos = s;
			for (Entry e : ranges) {
				inputs.add(new Input(e.toCircuitString(), this, width, ypos, e
						.getSize()));
				ypos += s;
			}

			// set up output
			outputs.add(new Output("output", this, 0,
					((ranges.size() - 1) / 2 + 1) * s, bits));
			if (loadTriState) {
				outputs.get(0).loadSetTriState();
			}
			...
			int xpos = s;
			for (Entry e : ranges) {
				inputs.add(new Input(e.toCircuitString(), this, xpos, 0, e
						.getSize()));
				xpos += s;
			}
			// set up output
			outputs.add(new Output("output", this,
					((ranges.size() - 1) / 2 + 1) * s, height, bits));
			...
			int xpos = s;
			for (Entry e : ranges) {
				inputs.add(new Input(e.toCircuitString(), this, xpos, height, e
						.getSize()));
				xpos += s;
			}
			// set up output
			outputs.add(new Output("output", this,
					((ranges.size() - 1) / 2 + 1) * s, 0, bits));
	...
	public void draw(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			for (Input input : inputs) {
				...
				g.drawString(input.getName(), edge, (int) (ypos - t.getHeight()
						/ 2 + fm.getAscent()));
				g.drawLine(x + width - s / 2, ypos,
						(int) (edge + t.getWidth() + d2), ypos);
			}

			// draw split line
			g.setColor(Color.BLACK);
			g.drawLine(x + width - s / 2, y + s, x + width - s / 2, y + height
					- s);
			...
			for (Input input : inputs) {
				...
				Rectangle2D t = fm.getStringBounds(input.getName(), g);
				g.setColor(Color.BLACK);
				int edge = (int) (x + width - JLSInfo.pointDiameter / 2 - t
						.getWidth());
				g.drawString(input.getName(), edge, (int) (ypos - t.getHeight()
						/ 2 + fm.getAscent()));
			...
			for (Input input : inputs) {
				...
				if (inum % 2 == 0) {
					g.drawString(input.getName(),
							xpos - (int) t.getWidth() / 2,
							(int) (edge + t.getHeight() / 2 + 6));
				}
				g.drawLine(xpos, y + height - s, xpos,
						(int) (edge + t.getHeight() + d2));
				inum++;
			}
	...
	/**
	 * Set this element to tri-state or not. Output will be tri-state if and
	 * only if all attached inputs are. Don't propagate if no change.
	 * 
	 * @param which
