...
public final class TextFilter extends DocumentFilter {
	...
	/**
	 * Set the maximum value for numbers allowed in this text field. Make sure
	 * base is set first.
	 * 
	 * @param max
	 *            The maximum value.
	...
	public void remove(DocumentFilter.FilterBypass b, int offset, int length)
		...
		if (length == 0) {
			...
			try {
				...
				if (max != null) {
					...
					String all = old.substring(0, offset) + str
							+ old.substring(offset + length);
