...
/**
 * Memory element, initialized internally or from a file. Implements either ROM
 * or SRAM (no DRAM).
 * 
...
public class Memory extends LogicElement {
	 ...
	 * using font info from graphics object. Note input positions: RAM-> 0 -
	 * addr, 1 - input, 2 - WE, 3 - OE, 4 - CS ROM-> 0 - addr, 1 - OE, 2 - CS
	 * 
	 * @param g
	...
	public void draw(Graphics g) {
		...
		ly = out.getY();
		g.setColor(Color.black);
		g.drawString("out", lx - fm.stringWidth("out") - d2, ly - h / 2
				+ ascent);
		out.draw(g);
	 ...
	 * 
	 * @param state
	 *            True to make it watched, false to make it not watched.
	 */
	...
	private class MemoryEdit extends JDialog implements ActionListener {
		...
		private JTextField capacityField = new JTextField(defaultCapacity + "",
				10);
		private KeyPad bitsPad = new KeyPad(bitsField, 10, bits, this);
		private KeyPad capacityPad = new KeyPad(capacityField, 10,
		 ...
		 * @param create
		 *            True if creating a new memory element, false if changing
		 *            one.
		 */
		private MemoryEdit(int x, int y, boolean create) {

			// set up window title
			super(JLSInfo.frame, create ? "Create Memory" : "Change Memory",
					true);
			...
			if (create) {
				...
				labels.add(bits);
				JLabel words = new JLabel("Capacity (words): ",
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == ok || event.getSource() == nameField
					|| event.getSource() == bitsField
					|| event.getSource() == capacityField) {
				...
				if (create) {
					try {
						...
						tcapacity = Integer.parseInt(capacityField.getText(),
								10);
					...
					if (ram.isSelected()) {
						...
						JOptionPane.showMessageDialog(this, "Pick RAM or ROM",
				...
				if (msg != null) {
					JOptionPane.showMessageDialog(getParent(), msg
							+ " in built in initialization", "Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				// get file name
				String file = JOptionPane.showInputDialog(this, "File name?",
						fileName);
				if (file == null)
					return;
				file = file.trim();
				while (!file.equals("") && !Util.isValidName(file)) {
					JOptionPane.showMessageDialog(this,
							...
							JOptionPane.ERROR_MESSAGE);
					file = JOptionPane.showInputDialog(this, "File name?",
							fileName);
					if (file == null)
						return;
					file = file.trim();
				...
				Action okAction = new AbstractAction("ok") {
					public void actionPerformed(ActionEvent event) {
						...
						String msg = initOK(tempInit, Integer.MAX_VALUE,
	 ...
	 * @param storing
	 *            True if values are to be stored, false if just checking.
	 * 
	 * @return null if syntax is ok, an error message if not.
	 */
	public String initOK(String str, int maxAddr, int bitsPerWord,
			boolean storing) {
		...
		while (scanning) {
			...
			if (!temp.equals("") && temp.charAt(0) != '#') {
				...
				if (bval.length() > bitsPerWord) {
					...
					lscan.close();
					return "line " + lineNumber
							+ ": value has more bits than word size";
				}
		...
		for (int addr : addrs) {
			...
			if (firstTime) {
				...
				if (qual.equals("")) {
					...
					System.out.println("Changed locations in memory " + qual
							+ "." + name);
				}
	...
	private java.util.List<WriteRecord> activity = new LinkedList<WriteRecord>();
			...
			try {
				...
				if (msg != null) {
					if (JLSInfo.batch && JLSInfo.frame == null
							&& !JLSInfo.isApplet) {
						System.out.println(msg + " in memory file " + name
								+ ", all zeros assumed");
					} else {
						JOptionPane.showMessageDialog(JLSInfo.frame, msg
								+ "in memory file " + name
								+ ", all zeros assumed", "Error",
								JOptionPane.ERROR_MESSAGE);
					}
				}
				...
				if (JLSInfo.batch && JLSInfo.frame == null && !JLSInfo.isApplet) {
					System.out.println("Initialization file for memory " + name
							+ " cannot be read, all zeros assumed");
				} else {
					JOptionPane.showMessageDialog(JLSInfo.frame,
							"Initialization file for memory " + name
									+ " cannot be read, all zeros assumed",
							"Error", JOptionPane.ERROR_MESSAGE);
				}
			}
	...
	public String getActivityTrace() {
		...
		for (WriteRecord rec : activity) {
			result += "0x" + BitSetUtils.ToString(rec.what, 16)
					...
					+ Integer.toString(rec.where, 16) + " at time " + rec.when
					+ "\n";
		}
		return result;
