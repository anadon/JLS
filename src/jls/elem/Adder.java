...
/**
 * An n-bit adder with carry in and carry out (n chosen by user). Propagation
 * delay is proportional to the number of bits.
 * 
...
public class Adder extends LogicElement {
	...
	public void draw(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.UP
			...
			g.drawLine(x + 3 * s / 2, y + 3 * s / 2, x + 5 * s / 2, y + 3 * s
					/ 2);
		} else if (orientation == JLSInfo.Orientation.LEFT
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			...
			g.drawString("A", x + d / 2, (int) (y + s - t.getHeight() / 2)
					+ ascent);
			t = fm.getStringBounds("B", g);
			g.drawString("B", x + d / 2, (int) (y + 3 * s - t.getHeight() / 2)
					+ ascent);
			...
			g.drawString("Cin", x + (int) (width - t.getWidth()) / 2, y
					+ ascent);
			t = fm.getStringBounds("Cout", g);
			g.drawString("Cout", x + (int) (width - t.getWidth()) / 2, y
					+ height - descent);
			g.setFont(f);
		} else if (orientation == JLSInfo.Orientation.LEFT) {
			int ascent = fm.getAscent();
			Rectangle2D t = fm.getStringBounds("A", g);
			g.drawString("A", (int) (x + width - t.getWidth() - d / 2),
					(int) (y + s - t.getHeight() / 2) + ascent);
			t = fm.getStringBounds("B", g);
			g.drawString("B", (int) (x + width - t.getWidth() - d / 2),
					(int) (y + 3 * s - t.getHeight() / 2) + ascent);
			t = fm.getStringBounds("S", g);
			...
			int descent = fm.getDescent();
			Rectangle2D t = fm.getStringBounds("A", g);
			g.drawString("A", (int) (x + s - t.getWidth() / 2),
					(int) (y + t.getHeight() / 4) + ascent);
			t = fm.getStringBounds("B", g);
			g.drawString("B", (int) (x + width - t.getWidth() - d / 2),
					(int) (y + t.getHeight() / 4) + ascent);
			t = fm.getStringBounds("S", g);
			g.drawString("S", x + (int) (width - t.getWidth()) / 2, y + height
					- descent);
			...
			t = fm.getStringBounds("Cout", g);
			g.drawString("Cout", x + (int) (width - t.getWidth() - 5), y
					+ height / 2 + ascent);
			g.setFont(f);
		} else if (orientation == JLSInfo.Orientation.UP) {
			int ascent = fm.getAscent();
			Rectangle2D t = fm.getStringBounds("A", g);
			g.drawString("A", (int) (x + s - t.getWidth() / 2), (int) (y
					+ height - t.getHeight())
					+ ascent);
			t = fm.getStringBounds("B", g);
			g.drawString("B", (int) (x + width - t.getWidth() - d / 2),
					(int) (y + height - t.getHeight()) + ascent);
			t = fm.getStringBounds("S", g);
			g.drawString("S", x + (int) (width - t.getWidth()) / 2, y
					+ (int) (t.getHeight()));
	...
	/**
	 * Tells if an adder is capable of rotatating, can only rotate when inputs
	...
	public boolean canRotate() {
		return !(inputs.get(0).isAttached() || inputs.get(1).isAttached()
				|| inputs.get(2).isAttached() || outputs.get(0).isAttached() || outputs
				.get(1).isAttached());
	}
	...
	/**
	 * Tells if an adder is capable of flipping, can only flip when inputs or
