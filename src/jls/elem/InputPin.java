...
public class InputPin extends Pin implements TriProp {
	...
	public void init(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			Output out = new Output("output", this, width, JLSInfo.spacing,
					bits);
			outputs.add(out);
			if (loadTriState) {
				out.loadSetTriState();
	...
	public void draw(Graphics g) {
		...
		new Rectangle(original.x, original.y, original.width - s,
				original.height);
	...
	/**
	 * Tells if a InputPin is capable of rotatating, can only rotate when output
	 * has no attachment.
	 * 
	...
	/**
	 * Tells if a InputPin is capable of flipping, can only flip when output has
	 * no attachment.
	 * 
	 * @return False if output has a wire attached, True otherwise
	 */
	...
	public void showCurrentValue(Point where) {
		...
		if (currentValue != null) {
			...
			value = "0x" + hex + " (" + unsigned + " unsigned, " + signed
					+ " signed)";
		}
