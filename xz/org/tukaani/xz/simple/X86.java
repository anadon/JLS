...
public final class X86 implements SimpleFilter {
	private static final boolean[] MASK_TO_ALLOWED_STATUS = { true, true, true,
	...
	public int code(byte[] buf, int off, int len) {
		...
		for (i = off; i <= end; ++i) {
			...
			if ((prevPos & ~3) != 0) { // (unsigned)prevPos > 3
				...
				if (prevMask != 0) {
					if (!MASK_TO_ALLOWED_STATUS[prevMask]
							|| test86MSByte(buf[i + 4
									- MASK_TO_BIT_NUMBER[prevMask]])) {
						prevPos = i;
			...
			if (test86MSByte(buf[i + 4])) {
				int src = (buf[i + 1] & 0xFF) | ((buf[i + 2] & 0xFF) << 8)
						| ((buf[i + 3] & 0xFF) << 16)
						| ((buf[i + 4] & 0xFF) << 24);
				int dest;
				while (true) {
