...
public final class Eula extends JDialog implements ActionListener {
	...
	/**
	 * Check for existing agreement file in user.home. If none there, then show
	 * frame, get ok, and save agreement file.
	 */
	...
	private static void presentEula() {
		if (!JLSInfo.batch) {
			try {
				new Eula((Frame) null, "License Agreement", true);
	...
	private static void presentOnTerminal() {
		System.out
				.println("Please read and accept the license agreement below.\n");
		...
		if (in.equalsIgnoreCase("y") || in.length() == 0)
			ok = true;
	}
	...
	public Eula(Frame f, String n, boolean m) {
		...
		for (int i = 0; i < lines.length; i += 1) {
			text.append(" "
					+ lines[i].replace("JLSInfo.year", JLSInfo.year + "")
					+ "\n");
		}
