...
public final class InterractiveSimulator extends Simulator {
	...
	public InterractiveSimulator() {
		...
		// show trace buttton
		JLabel showTrace = new JLabel(
				"<html>Drag divider up to see signal traces</html>",
		...
		sim = new Thread("Runner") {
			...
			public void run() {
				...
				while (!stopping && !eventQueue.isEmpty() && now <= maxTime) {
					...
					// if after step end time ... (can't happen in JLSInfo.batch
					// mode)
	...
	/**
	 * Pause or resume the simulation, if there is one. Called by the pause
	 * element.
	 * 
	 * @param which
	 *            True to pause the simulation, false to resume it.
	 */
	...
	public void stop() {
		...
		if (sim != null) {
			/*
			 * action.removeAll(); action.add(start); action.add(step);
			 * action.add(animate); action.add(print); action.add(help);
			 * action.validate();
			 */
			stopping = true;
			stepEnd = -1; // kill any stepping
	...
	/**
	 * Find all probes and watched elements, add them to Traces window. If an
	 * element is not watched, reset its trace position. Descends into
	 * subcircuits recursively.
	 * 
	 * @param circ
	 *            The circuit to find the traces in.
	 */
		...
		for (Element element : circ.getElements()) {
			...
			if (element instanceof Wire) {
				...
				if (wire.hasProbe()) {
					...
					Trace tr = new Trace(name, element, wire.getBits(),
	...
	/**
	 * Update the status bar. Grabs current values of msg and showClock and puts
	 * into status bar labels.
	 */
	...
	public class Traces extends JPanel implements MouseMotionListener {
		...
		public void setup() {
			...
			// add traces to display, figure out space for longest name, and
			// update
		...
		/**
		 * Move a trace up or down. Also rebuild trace position map.
		 * 
		 * @param tr
		 *            The trace to move
		 * @param dir
		 *            The direction (-2=top, -1=up, 1=down, 2=bottom).
		 */
		...
		/**
		 * Tell all the traces that the mouse has moved in the trace window, but
		 * only if the simulator is paused or stopped.
		 * 
		 * @param event
		 *            The event containing the new position of the mouse.
		 */
	...
	/**
	 * A header for the traces, will display the slider and the time the slider
	 * indicates.
	 */
	@SuppressWarnings("serial")
