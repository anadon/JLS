...
| CAP-11 | required - **ADMISSION SLICE ONLY** | the analog port permit and the datum/node-partition contract, without which FEAT-048 has no legal terminal to hang a bridge on: at `2d0ca9d` `src/jls/elem/Element.java:17-18` permits only `DisplayElement, LogicElement, Wire` and `src/jls/elem/Put.java:57` takes a `LogicElement`. **Nothing else of this feature is needed there** - the model library, the drawn palette and the convergence hardening stay funded by CAP-12. Added 2026-08-03 under D16: this table had **no CAP-11 row at all** and `CAP-11-audio-input.md` read "Deliberately absent: FEAT-049", while the filed CAP-11 issue #303 carries 331 in `requires_features` and this feature's own issue #331 declares `serves_capstones` including 303. Both filed ends said required; the two documents were the outliers and are corrected together. **The slice has no band** - this feature is priced whole at 21-33 mw and the corpus does not decompose it - so CAP-11's required sum is a lower bound until it does, and the slice is counted once, in CAP-12 |
