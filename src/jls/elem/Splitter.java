...
public class Splitter extends Group implements TriProp {
	...
	public void init(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			// set up input
			inputs.add(new Input("input", this, 0,
					((ranges.size() - 1) / 2 + 1) * s, bits));

			// set up outputs
			int ypos = s;
			for (Entry e : ranges) {
				Output out = new Output(e.toCircuitString(), this, width, ypos,
						e.getSize());
				outputs.add(out);
				if (loadTriState) {
					out.loadSetTriState();
			...
			// set up input
			inputs.add(new Input("input", this, width,
					((ranges.size() - 1) / 2 + 1) * s, bits));

			// set up outputs
			int ypos = s;
			for (Entry e : ranges) {
				Output out = new Output(e.toCircuitString(), this, 0, ypos,
						e.getSize());
				outputs.add(out);
				if (loadTriState) {
					out.loadSetTriState();
			...
			int xpos = s;
			for (Entry e : ranges) {
				Output out = new Output(e.toCircuitString(), this, xpos, 0,
						e.getSize());
				outputs.add(out);
				if (loadTriState) {
					out.loadSetTriState();
			...
			// set up input
			inputs.add(new Input("input", this, ((ranges.size() - 1) / 2 + 1)
					* s, height, bits));
		} else if (orientation == JLSInfo.Orientation.DOWN) {
			int xpos = s;
			for (Entry e : ranges) {
				Output out = new Output(e.toCircuitString(), this, xpos,
						height, e.getSize());
				outputs.add(out);
				if (loadTriState) {
					out.loadSetTriState();
	...
	public void draw(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.RIGHT) {
			...
			for (Output output : outputs) {
				...
				g.drawString(output.getName(), edge,
						(int) (ypos - t.getHeight() / 2 + fm.getAscent()));
			...
			for (Output output : outputs) {
				...
				if (inum % 2 == 0) {
					g.drawString(output.getName(), xpos - (int) t.getWidth()
							/ 2, (int) (edge - t.getHeight() / 2 + 6));
				}
