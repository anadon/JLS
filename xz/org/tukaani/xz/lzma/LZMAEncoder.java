...
public abstract class LZMAEncoder extends LZMACoder {
	 ...
	 * bytes.
	 */
	private static final int LZMA2_UNCOMPRESSED_LIMIT = (2 << 20)
			- MATCH_LEN_MAX;
	...
	public static int getMemoryUsage(int mode, int dictSize,
		...
		case MODE_NORMAL:
			m += LZMAEncoderNormal
					.getMemoryUsage(dictSize, extraSizeBefore, mf);
			break;
	...
	public static LZMAEncoder getInstance(RangeEncoder rc, int lc, int lp,
			int pb, int mode, int dictSize, int extraSizeBefore, int niceLen,
			int mf, int depthLimit) {
		switch (mode) {
		case MODE_FAST:
			return new LZMAEncoderFast(rc, lc, lp, pb, dictSize,
					extraSizeBefore, niceLen, mf, depthLimit);

		case MODE_NORMAL:
			return new LZMAEncoderNormal(rc, lc, lp, pb, dictSize,
					extraSizeBefore, niceLen, mf, depthLimit);
		}
	...
	LZMAEncoder(RangeEncoder rc, LZEncoder lz, int lc, int lp, int pb,
			int dictSize, int niceLen) {
		super(pb);
		this.rc = rc;
		this.lz = lz;
		...
		if (distSlot >= DIST_MODEL_START) {
			...
			if (distSlot < DIST_MODEL_END) {
				rc.encodeReverseBitTree(
						distSpecial[distSlot - DIST_MODEL_START], distReduced);
			} else {
				rc.encodeDirectBits(distReduced >>> ALIGN_BITS, footerBits
						- ALIGN_BITS);
	...
	int getShortRepPrice(int anyRepPrice, State state, int posState) {
		return anyRepPrice
				...
				+ RangeEncoder
						.getBitPrice(isRep0Long[state.get()][posState], 0);
	}
		...
		if (rep == 0) {
			...
			if (rep == 1)
				...
				price += RangeEncoder.getBitPrice(isRep1[state.get()], 1)
						+ RangeEncoder
								.getBitPrice(isRep2[state.get()], rep - 2);
		}
	...
	int getMatchAndLenPrice(int normalMatchPrice, int dist, int len,
			int posState) {
	...
	private void updateDistPrices() {
		...
		for (int distState = 0; distState < DIST_STATES; ++distState) {
			for (int distSlot = 0; distSlot < distSlotPricesSize; ++distSlot)
				distSlotPrices[distState][distSlot] = RangeEncoder
						.getBitTreePrice(distSlots[distState], distSlot);

			for (int distSlot = DIST_MODEL_END; distSlot < distSlotPricesSize; ++distSlot) {
				int count = (distSlot >>> 1) - 1 - ALIGN_BITS;
				distSlotPrices[distState][distSlot] += RangeEncoder
						.getDirectBitsPrice(count);
			}
		...
		for (int distSlot = DIST_MODEL_START; distSlot < DIST_MODEL_END; ++distSlot) {
			...
			for (int i = 0; i < limit; ++i) {
				...
				for (int distState = 0; distState < DIST_STATES; ++distState)
					fullDistPrices[distState][dist] = distSlotPrices[distState][distSlot]
							+ price;
	...
	class LiteralEncoder extends LiteralCoder {
		...
		void encode() {
			assert readAhead >= 0;
			int i = getSubcoderIndex(lz.getByte(1 + readAhead), lz.getPos()
					- readAhead);
			subencoders[i].encode();
		}

		int getPrice(int curByte, int matchByte, int prevByte, int pos,
				State state) {
			int price = RangeEncoder.getBitPrice(isMatch[state.get()][pos
					& posMask], 0);
	...
	class LengthEncoder extends LengthCoder {
		...
		LengthEncoder(int pb, int niceLen) {
			...
			int lenSymbols = Math.max(niceLen - MATCH_LEN_MIN + 1, LOW_SYMBOLS
					+ MID_SYMBOLS);
		...
		private void updatePrices(int posState) {
			...
			for (; i < LOW_SYMBOLS + MID_SYMBOLS; ++i)
				prices[posState][i] = choice0Price
						+ choice1Price
						+ RangeEncoder.getBitTreePrice(mid[posState], i
								- LOW_SYMBOLS);
