...
public class SeekableXZInputStream extends SeekableInputStream {
	 ...
	 * 
	 * @param in
	 *            seekable input stream containing one or more XZ Streams; the
	 ...
	 *             XZ headers seem valid but they specify options not supported
	 *             by this implementation
	 * 
	 * @throws EOFException
	 *             less than 6 bytes of input was available from <code>in</code>
	 *             , or (unlikely) the size of the underlying stream got smaller
	 *             while this was reading from it
	 * 
	 * @throws IOException
		...
		while (pos > 0) {
			...
			try {
				...
				throw new MemoryLimitException(e.getMemoryNeeded()
						+ indexMemoryUsage, memoryLimit + indexMemoryUsage);
			}
			...
			if (blockCount < 0)
				throw new UnsupportedOptionsException("XZ file has over "
	 ...
	 * @throws XZIOException
	 *             if <code>blockNumber&nbsp;&lt;&nbsp;0</code> or
	 *             <code>blockNumber&nbsp;&gt;=&nbsp;getBlockCount()</code>, or
	...
	private void locateBlockByNumber(BlockInfo info, int blockNumber) {
		// Validate.
		if (blockNumber < 0 || blockNumber >= blockCount)
			throw new IndexOutOfBoundsException("Invalid XZ Block number: "
					+ blockNumber);
