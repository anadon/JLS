...
public class LZMA2Options extends FilterOptions {
	...
	private static final int[] presetToDictSize = { 1 << 18, 1 << 20, 1 << 21,
			1 << 22, 1 << 22, 1 << 23, 1 << 23, 1 << 24, 1 << 25, 1 << 26 };
	...
	public void setPreset(int preset) throws UnsupportedOptionsException {
		if (preset < 0 || preset > 9)
			throw new UnsupportedOptionsException("Unsupported preset: "
					+ preset);
	...
	public void setDictSize(int dictSize) throws UnsupportedOptionsException {
		if (dictSize < DICT_SIZE_MIN)
			throw new UnsupportedOptionsException(
					"LZMA2 dictionary size must be at least 4 KiB: " + dictSize
							+ " B");

		if (dictSize > DICT_SIZE_MAX)
			throw new UnsupportedOptionsException(
					"LZMA2 dictionary size must not exceed "
							+ (DICT_SIZE_MAX >> 20) + " MiB: " + dictSize
							+ " B");
	 ...
	 * 
	 * @throws UnsupportedOptionsException
	 *             <code>lc</code> is invalid, or the sum of <code>lc</code> and
	 *             <code>lp</code> exceed LC_LP_MAX
	 */
	 ...
	 * 
	 * @throws UnsupportedOptionsException
	 *             <code>lp</code> is invalid, or the sum of <code>lc</code> and
	 *             <code>lp</code> exceed LC_LP_MAX
	 */
	...
	public void setPb(int pb) throws UnsupportedOptionsException {
		if (pb < 0 || pb > PB_MAX)
			throw new UnsupportedOptionsException("pb must not exceed "
					+ PB_MAX + ": " + pb);
	...
	public void setNiceLen(int niceLen) throws UnsupportedOptionsException {
		if (niceLen < NICE_LEN_MIN)
			throw new UnsupportedOptionsException(
					"Minimum nice length of matches is " + NICE_LEN_MIN
							+ " bytes: " + niceLen);
	...
	public void setMatchFinder(int mf) throws UnsupportedOptionsException {
		if (mf != MF_HC4 && mf != MF_BT4)
			throw new UnsupportedOptionsException("Unsupported match finder: "
					+ mf);
	...
	public int getEncoderMemoryUsage() {
		return (mode == MODE_UNCOMPRESSED) ? UncompressedLZMA2OutputStream
				.getMemoryUsage() : LZMA2OutputStream.getMemoryUsage(this);
	}
