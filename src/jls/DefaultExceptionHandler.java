...
public final class DefaultExceptionHandler implements
	...
	/**
	 * Show a dialog and print a stack trace to a file. While debugging print a
	 * stack trace to the console.
	 * 
	 * @param t
	 *            Unused.
	 * @param th
	 *            A throwable containing the exception/error information.
	 */
	public void uncaughtException(Thread t, Throwable th) {
		...
		// all other errors/exceptions...
			...
			if (jls == null) {
				...
				System.out
						.println("JLS will create a file called JLSerror in the current folder/directory.");
				System.out
						.println("Please email it to pop@mtu.edu to report the error so it can be fixed.");
				System.exit(1);
			}
	...
	protected void saveTrace(Throwable th) {
		...
		try {
			...
			out.println("Thanks.");
			out.printf("JLS %d.%d.%d %s\n", JLSInfo.vers, JLSInfo.release,
					JLSInfo.buildNum, JLSInfo.build);
