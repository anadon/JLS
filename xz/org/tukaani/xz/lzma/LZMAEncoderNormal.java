...
final class LZMAEncoderNormal extends LZMAEncoder {
	...
	static int getMemoryUsage(int dictSize, int extraSizeBefore, int mf) {
		return LZEncoder.getMemoryUsage(dictSize,
				Math.max(extraSizeBefore, EXTRA_SIZE_BEFORE), EXTRA_SIZE_AFTER,
				MATCH_LEN_MAX, mf) + OPTS * 64 / 1024;
	}

	LZMAEncoderNormal(RangeEncoder rc, int lc, int lp, int pb, int dictSize,
			int extraSizeBefore, int niceLen, int mf, int depthLimit) {
		super(rc, LZEncoder.getInstance(dictSize,
				Math.max(extraSizeBefore, EXTRA_SIZE_BEFORE), EXTRA_SIZE_AFTER,
				niceLen, MATCH_LEN_MAX, mf, depthLimit), lc, lp, pb, dictSize,
				niceLen);
	...
	int getNextSymbol() {
		...
		for (int rep = 0; rep < REPS; ++rep) {
			...
			int longRepPrice = getLongRepPrice(anyRepPrice, rep, state,
					posState);
			do {
				int price = longRepPrice
						+ repLenEncoder.getPrice(repLen, posState);
		...
		// Calculate the prices of normal matches that are longer than rep0.
		{
			...
			if (len <= mainLen) {
				...
				while (true) {
					...
					int price = getMatchAndLenPrice(normalMatchPrice, dist,
							len, posState);
		...
		while (++optCur < optEnd) {
			...
			if (avail >= MATCH_LEN_MIN) {
				int startLen = calcLongRepPrices(pos, posState, avail,
						anyRepPrice);
				if (matches.count > 0)
					calcNormalMatchPrices(pos, posState, avail, anyMatchPrice,
							startLen);
			}
		}
	...
	private void updateOptStateAndReps() {
		...
		if (optPrev == optCur - 1) {
			...
			if (back < REPS) {
				...
				System.arraycopy(opts[optPrev].reps, 0, opts[optCur].reps, 1,
						REPS - 1);
			}
		}
	}
	 ...
	 */
	private void calc1BytePrices(int pos, int posState, int avail,
			int anyRepPrice) {
		...
		// Try a short rep.
		if (matchByte == curByte
				&& (opts[optCur + 1].optPrev == optCur || opts[optCur + 1].backPrev != 0)) {
		...
		if (!nextIsByte && matchByte != curByte && avail > MATCH_LEN_MIN) {
			...
			if (len >= MATCH_LEN_MIN) {
				...
				int price = literalPrice
						+ getLongRepAndLenPrice(0, len, nextState, nextPosState);
	 ...
	 */
	private int calcLongRepPrices(int pos, int posState, int avail,
			int anyRepPrice) {
	...
	private void calcNormalMatchPrices(int pos, int posState, int avail,
		...
		for (int len = startLen;; ++len) {
			...
			// possible distance.
			int matchAndLenPrice = getMatchAndLenPrice(normalMatchPrice, dist,
					len, posState);
			...
			if (len2 >= MATCH_LEN_MIN) {
				...
				int price = matchAndLenPrice
						+ literalEncoder.getPrice(curByte, matchByte, prevByte,
								pos + len, nextState);
				nextState.updateLiteral();
