...
class GridTransformTest {
	...
	void andGateOrientationsAreTransformsOfEachOther() {
		...
		assertEquals(new Point(48, 24), GridTransform.mirrorX(0, 24, w));
		assertEquals(new Point(0, 12), GridTransform.mirrorX(48, 12, w));

		// up = rotateCCW(right): input0@0,48  input1@24,48  output@12,0
		assertEquals(new Point(0, 48), GridTransform.rotateCCW(0, 0, w));
		assertEquals(new Point(24, 48), GridTransform.rotateCCW(0, 24, w));
		assertEquals(new Point(12, 0), GridTransform.rotateCCW(48, 12, w));

		// down = mirrorY(up), in the rotated 24x48 frame:
		// input0@0,0  input1@24,0  output@12,48
		assertEquals(new Point(0, 0), GridTransform.mirrorY(0, 48, w));
		assertEquals(new Point(24, 0), GridTransform.mirrorY(24, 48, w));
		assertEquals(new Point(12, 48), GridTransform.mirrorY(12, 0, w));
	}
	...
	void adderLeftIsMirrorOfRight() {
		...
		int w = 48;
		assertEquals(new Point(48, 12), GridTransform.mirrorX(0, 12, w));
		assertEquals(new Point(48, 36), GridTransform.mirrorX(0, 36, w));
		assertEquals(new Point(24, 0), GridTransform.mirrorX(24, 0, w));
		assertEquals(new Point(24, 48), GridTransform.mirrorX(24, 48, w));
		assertEquals(new Point(0, 24), GridTransform.mirrorX(48, 24, w));
	}
	...
	void quarterTurnsCompose() {
		...
		for (int px = 0; px <= w; px += 12) {
			for (int py = 0; py <= h; py += 12) {
				Point cw = GridTransform.rotateCW(px, py, h);
				assertEquals(new Point(px, py),
						GridTransform.rotateCCW(cw.x, cw.y, h));
				Point cw2 = GridTransform.rotateCW(cw.x, cw.y, w);
