 ...
 * random-accessible .xz file.
 * <p>
 * Arguments: [preset [block size]]
 * <p>
 * Preset is an LZMA2 preset level which is an integer in the range [0, 9]. The
 * default is 6.
 * <p>
 * Block size specifies the amount of uncompressed data to store per XZ Block.
 * The default is 1 MiB (1048576 bytes). Bigger means better compression ratio.
 * Smaller means faster random access.
 */
class XZSeekEncDemo {
	public static void main(String[] args) throws Exception {
		...
		options.setDictSize(Math.min(options.getDictSize(),
				Math.max(LZMA2Options.DICT_SIZE_MIN, blockSize)));
