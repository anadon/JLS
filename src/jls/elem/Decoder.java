...
public class Decoder extends LogicElement {
	...
	public void draw(Graphics g) {
		...
		if (orientation == JLSInfo.Orientation.LEFT
			...
			g.drawString(inout, x + (int) (width - t.getWidth()) / 2, y
					+ (int) (height / 2 - t.getHeight()) / 2 + fm.getAscent());
			t = fm.getStringBounds(dec, g);
			g.drawString(dec, x + (int) (width - t.getWidth()) / 2, y
					+ JLSInfo.spacing + (int) (height / 2 - t.getHeight()) / 2
					+ fm.getAscent());
		} else if (orientation == JLSInfo.Orientation.UP) {
			Rectangle2D t = fm.getStringBounds("1", g);
			g.drawString("1", x + (width - (int) t.getWidth()) / 2,
					y + fm.getHeight() + 5);
			g.drawString("|", x + (width - (int) t.getWidth()) / 2,
					y + fm.getHeight() + 20);
			g.drawString("n", x + (width - (int) t.getWidth()) / 2,
					y + fm.getHeight() + 35);
		} else if (orientation == JLSInfo.Orientation.DOWN) {
			Rectangle2D t = fm.getStringBounds("1", g);
			g.drawString("n", x + (width - (int) t.getWidth()) / 2,
	...
	/**
	 * Tells if a decoder is capable of rotatating, can only rotate when inputs
	...
	/**
	 * Tells if a decoder is capable of flipping, can only flip when inputs or
