...
public class SubCircuit extends LogicElement implements TriProp {
	...
	/**
	 * Get the (local) name of this subcircuit element (not the circuit it
	 * represents).
	 * 
	 * @return the name.
	 */
	 ...
	 * @param editWindow
	 *            The editor window this subcircuit will be displayed in.
	 * @param x
		...
		for (Pin el : pins) {
			if (el instanceof InputPin) {
				...
				if (orientation == JLSInfo.Orientation.RIGHT) {
					in = new Input(pin.getName(), this, 0, height,
							pin.getBits());
				} else {
					in = new Input(pin.getName(), this, width, height,
							pin.getBits());
				}
				inputs.add(in);
				height += s;
				...
				Output out;
				if (orientation == JLSInfo.Orientation.RIGHT) {
					out = new Output(pin.getName(), this, width, height,
							pin.getBits());
				} else {
					out = new Output(pin.getName(), this, 0, height,
							pin.getBits());
				}
				outputs.add(out);
				height += s;
	 ...
	 * 
	 * @param which
	 *            True to set all watchable elements to watched, false to make
	 *            them not watched.
	 */
	...
	/**
	 * Remap all input and output pins to inputs and outputs. Used by undo/redo
	 * (finishDo).
	 * 
	 * @param circuit
	 *            The new circuit containing new versions of the input and
	 *            output pins.
	 */
	 ...
	 * 
	 * @param which
	 *            True to set to tri-state, false otherwise.
	 */
	...
	/**
	 * See if this element can be flipped. It can be if none of its inputs or
	 * outputs are attached yet.
	 * 
	 ...
	 * 
	 * @param g
	 *            The Graphics object used to draw this element.
	 */
	...
	/**
	 * Initialize this element by setting all output pins of this element to 0
	 * or null and initializing all elements in the subcircuit.
	 * 
	 * @param sim
	 *            The simulator for this circuit.
	 */
