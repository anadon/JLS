...
public class Clock extends LogicElement {
	...
	public void showInfo(JLabel info) {
		...
		info.setText("clock, cycle time = " + cycleTime + " (zero for "
				+ (cycleTime - oneTime) + ", one for " + oneTime + ")");
	...
	/**
	 * Tells if a clock is capable of rotatating, can only rotate when output
	 * has no attachment.
	 * 
	...
	/**
	 * Tells if a clock is capable of flipping, can only flip when output has no
	 * attachment.
	 * 
	 * @return False if output has a wire attached, True otherwise
	 */
	...
	private class ClockCreate extends JDialog implements ActionListener {
		...
		private KeyPad cycleTimePad = new KeyPad(cycleTimeField, 10,
				defaultCycleTime, this);
		private KeyPad oneTimePad = new KeyPad(oneTimeField, 10,
				defaultOneTime, this);
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == ok || event.getSource() == cycleTimeField
					|| event.getSource() == oneTimeField) {
				try {
					int newCycleTime = Integer.parseInt(cycleTimeField
							.getText());
