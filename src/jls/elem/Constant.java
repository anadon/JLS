...
public class Constant extends LogicElement implements ActionListener {
	...
	public void init(Graphics g) {
		...
		if (g != null) {
			...
			if (width == 0 && height == 0) {
				...
				width = Math.max((w + s / 2) / s * s, 2 * s); // ceiling in
																// spacings
				if (orientation == JLSInfo.Orientation.LEFT
						|| orientation == JLSInfo.Orientation.RIGHT) {
					height = 0; // not really, but bounding rectangle will be
								// large enough
				} else {
					height = 2 * s;
				}
	 ...
	 * compatibility with old versions of JLS that saved the value of the
	 * constant as a long, not as a String (i.e., BigInteger).
	 * 
	 * @param name
	 *            The instance variable name.
	 * @param value
	...
	/**
	 * Tells if a constant is capable of rotatating, can only rotate when output
	 * has no attachment.
	 * 
	...
	/**
	 * Tells if a constant is capable of flipping, can only flip when output has
	 * no attachment.
	 * 
	 * @return False if output has a wire attached, True otherwise
	 */
	 ...
	 * of gates). Used by all simple gates (nand, and, nor, or, xor, not).
	 */
	...
	private class ConstantCreate extends JDialog implements ActionListener {
		...
		private JTextField valueField = new JTextField(defaultValue + "",
				defaultBase);
		private KeyPad valuePad = new KeyPad(valueField, 16,
				defaultValue.longValue(), this);
	...
	/**
	 * Display dialog letting user change the value of the constant. New value
	 * must fit in current element.
	 */
	@SuppressWarnings("serial")
	private class ConstantChange extends JDialog implements ActionListener {
		...
		private JButton ok = new JButton("OK");
		private JButton cancel = new JButton("Cancel");
		private JTextField valueField = new JTextField(Util.convert(value,
				base, false), 10);
		private KeyPad valuePad = new KeyPad(valueField, 16,
				defaultValue.longValue(), this);
		private JRadioButton base2 = new JRadioButton("2");
	 ...
	 * 
	 * @param event
	 *            The event object for the button push.
	 */
	public void actionPerformed(ActionEvent event) {
