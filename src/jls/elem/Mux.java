...
public class Mux extends LogicElement {
	...
	public void init(Graphics g) {
		...
		if (outputOrientation == JLSInfo.Orientation.RIGHT) {
			...
			// create output
			outputs.add(new Output("output", this, width, (numInputs + 1) / 2
					* s, bits));
			...
			// create output
			outputs.add(new Output("output", this, 0, (numInputs + 1) / 2 * s,
					bits));
			...
			// create output
			outputs.add(new Output("output", this, (numInputs + 1) / 2 * s,
					height, bits));
			...
			// create output
			outputs.add(new Output("output", this, (numInputs + 1) / 2 * s, 0,
					bits));
		}
	...
	public void draw(Graphics g) {
		...
		if (outputOrientation == JLSInfo.Orientation.LEFT
			...
			for (Input input : inputs) {
				...
				if (inum >= 0) {
					g.setColor(Color.BLACK);
					if (outputOrientation == JLSInfo.Orientation.RIGHT) {
						g.drawString(inum + "", x + d2, input.getY() - hi / 2
								+ ascent);
					} else if (outputOrientation == JLSInfo.Orientation.LEFT) {
						g.drawString(inum + "", x + width - 5 * d2,
								input.getY() - hi / 2 + ascent);
					} else if (outputOrientation == JLSInfo.Orientation.UP
		...
		if (outputOrientation == JLSInfo.Orientation.UP
				|| outputOrientation == JLSInfo.Orientation.DOWN) {
			if (inputs.size() == 3) {
				...
				for (Input input : inputs) {
					...
					if (inum >= 0 && inum % 2 == 0) {
						g.setColor(Color.BLACK);
						if (outputOrientation == JLSInfo.Orientation.RIGHT) {
							g.drawString(inum + "", x + d2, input.getY() - hi
									/ 2 + ascent);
						} else if (outputOrientation == JLSInfo.Orientation.LEFT) {
							g.drawString(inum + "", x + width - 5 * d2,
									input.getY() - hi / 2 + ascent);
								...
								|| outputOrientation == JLSInfo.Orientation.DOWN) {
							g.drawString(inum + "", input.getX() - 4, y + 5
									* d2);
						}
					}
					inum += 1;
	...
	/**
	 * Tells if a mux is capable of flipping, can only flip when inputs or
	...
	/**
	 * Tells if a mux is capable of rotatating, can only rotate when inputs or
	 * outputs have no attachments.
	 * 
	 * @return False if any input or output has a wire attached, True otherwise
	 */
	 ...
	 * @param todo
	 *            Null if an input change, the new output value otherwise.
	 */
