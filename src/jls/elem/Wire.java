...
public class Wire extends Element {
	...
	/**
	 * A wire doesn't move on its own. It is drawn using the position of its
	 * ends.
	 * 
	 * @param dx
	 ...
	 * 
	 * @return true if the point is within 1/2 spacing of the wire and not
	 *         within pointDiameter pixels of either end, false if not.
	 */
	public boolean contains(int x, int y) {
		...
		if (Line2D.ptSegDist(x1, y1, x2, y2, x, y) < JLSInfo.spacing / 2
				&& Point2D.distance(x1, y1, x, y) > d
				&& Point2D.distance(x2, y2, x, y) > d) {
			return true;
		} else {
			return false;
		}
	...
	public boolean intersects(Rectangle rect) {
		...
		if (r.intersectsLine(x1, y1, x2, y2)) {
			...
			// then exclude end points (smaller rectangle a kludge 'cause
			// contains
			// means inside, not including on the edge)
			Rectangle rs = new Rectangle(rect.x - 1, rect.y - 1,
	...
	/**
	 * See if a given wire end touches the wire. The wire end must be within
	 * half a point diameter of the wire but not within a half point diameter of
	 * either end.
	 * 
	 * @param end
	 *            The wire end.
	...
	public void showInfo(JLabel info) {
		...
		if (!net.hasInput()) {
			...
			value = ", value = "
					+ BitSetUtils.toDisplay(getValue(), net.getBits());
		}
	...
	/**
	 * Remove this wire and both of its ends from the circuit. Ends are removed
	 * only if possible.
	 * 
	 * @param The
	 *            circuit it is in.
	...
	public boolean isInside(Rectangle rect) {
		...
		if (rect.contains(end1.getX(), end1.getY())
				&& rect.contains(end2.getX(), end2.getY())) {
			return true;
		} else {
			return false;
		}
	 ...
	 * 
	 * @param which
	 *            True to say it is, false if not.
	 */
	...
	public int length() {
		...
		return (int) (Point.distance(end1.getX(), end1.getY(), end2.getX(),
				end2.getY()));
	 ...
	 * 
	 * @param name
	 *            The name of the probe. If null, then prompt user for a name.
	 */
