...
/**
 * Superclass of all simple gates (AND, OR, etc). Contains common info and
 * method.
 * 
...
public abstract class Gate extends LogicElement {
	...
	/**
	 * Create a new Gate object. Subclass constructors do most of the work.
	 * 
	 * @param circuit
	 */
	 ...
	 * @param editWindow
	 *            The editor window this circuit is displayed in.
	 * @param x
	...
	/**
	 * Initialize internal info for this element. Sets up size, inputs and
	 * outputs.
	 * 
	 * @param g
	 *            Unused.
	 ...
	 *            The output writer.
	 * @param type
	 *            The type of gate (e.g., "AND"), all capitals.
	 * @param triState
	 *            If true, save that this element has a tri-state output.
	 */
	 ...
	 * @param type
	 *            The type of element (e.g., "AND"), all capitals.
	 */
	...
	/**
	 * Get the rectangle bounding this element. For gates this will be 1/2 space
	 * higher and lower (for left/right gates) or 1/2 space wider on each side
	 * (for up/down gates).
	 * 
	...
	public Rectangle getRect() {
		...
		if (orientation == Orientation.left || orientation == Orientation.right) {
			return new Rectangle(x, y - JLSInfo.spacing / 2, width, height
					+ JLSInfo.spacing);
		} else {
			return new Rectangle(x - JLSInfo.spacing / 2, y, width
					+ JLSInfo.spacing, height);
		}
	...
	/**
	 * Tells if a gate is capable of rotating, can only rotate when inputs or
	 * outputs have no attachments.
	 * 
	...
	/**
	 * Tells if a gate is capable of flipping, can only flip when inputs or
	 * outputs have no attachments.
	 * 
	 * @return False if any input or output has a wire attached, True otherwise
	 */
	...
	/**
	 * Dialog box to set multi-input gate parameters (number of inputs, number
	 * of gates). Used by all simple gates (nand, and, nor, or, xor).
	 */
	...
	protected class GateCreate extends JDialog implements ActionListener {
		...
		private KeyPad inputsPad = new KeyPad(inputsField, 10, defaultInputs,
				this);
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == ok || event.getSource() == inputsField
					|| event.getSource() == gatesField) {
				try {
					numInputs = Integer.parseInt(inputsField.getText());
					bits = Integer.parseInt(gatesField.getText());
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(this, "Value not numeric",
