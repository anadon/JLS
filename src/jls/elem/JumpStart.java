...
public class JumpStart extends LogicElement implements TriProp {
		...
		if (g != null) {
			...
			if (width == 0 && height == 0) {
				...
				width = Math.max((w + s / 2) / s * s, 2 * s); // ceiling in
																// spacings
				height = 0; // not really, but bounding rectangle will be large
							// enough
			}
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == ok || event.getSource() == nameField) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this, "Bits not numeric",
							"Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				if (bits < 1) {
	...
	/**
	 * Set this element to tri-state or not. Propagate tri-state property to the
	 * other end(s) of the jump.
	 * 
	 * @param which
	...
	/**
	 * Initialize this element by creating a set of all matching jump ends and
	 * setting the current value to 0.
	 * 
	 * @param sim
	 *            Unused.
	 */
