...
/**
 * Superclass of input and output pins. Contains common data and methods.
 * 
...
public abstract class Pin extends LogicElement {
	...
	public String toString() {
		...
		return name + ",bits=" + bits + ",watched=" + watched + ",hashCode="
				+ hashCode();
	...
	/**
	 * Initialize internal info for this element. Computes the size of the pin
	 * as a function of the name.
	 * 
	 * @param g
	 *            Graphics object used to compute the size of the name.
	 */
		...
		if (g != null) {
			...
			if (width == 0 && height == 0) {
				...
				if (orientation == JLSInfo.Orientation.LEFT
					...
					width = Math.max((w + s / 2) / s * s, 2 * s) + s; // ceiling
																		// in
																		// spacings
					height = 2 * s;
				} else {
	...
	/**
	 * Remove this element. Take name out of circuit's list of names used.
	 */
