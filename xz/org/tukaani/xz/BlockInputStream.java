...
class BlockInputStream extends InputStream {
	...
	public BlockInputStream(InputStream in, Check check, int memoryLimit,
			long unpaddedSizeInIndex, long uncompressedSizeInIndex)
		...
		ByteArrayInputStream bufStream = new ByteArrayInputStream(buf, 2,
				headerSize - 6);
		...
		if (unpaddedSizeInIndex != -1) {
			...
			// the Block Header.
			long compressedSizeFromIndex = unpaddedSizeInIndex
					- headerAndCheckSize;
		...
		for (int i = 0; i < filters.length; ++i) {
			if (filterIDs[i] == LZMA2Coder.FILTER_ID)
				...
				throw new UnsupportedOptionsException("Unknown Filter ID "
						+ filterIDs[i]);
		}
