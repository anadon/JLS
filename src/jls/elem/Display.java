...
public class Display extends LogicElement {
	...
	public void init(Graphics g) {
		...
		if (g != null) {
			...
			if (width == 0 && height == 0) {
				...
				BigInteger maxVal = new BigInteger("1").shiftLeft(bits)
	...
	public void draw(Graphics g) {
		...
		g.drawString(value, x + (width - sw) / 2, y + (height - hi) / 2
				+ ascent);
	...
	public void save(PrintWriter output) {
		...
		if (orient > 0)
			output.println(" int orient " + orient);
		output.println("END");
	} // end of save method
	...
	public void showInfo(JLabel info) {
		...
		info.setText(bits + " bit display, value = "
	...
	/**
	 * Tells if a display is capable of rotatating, can only rotate when input
	 * has no attachment.
	 * 
	 * @return False if input has a wire attached, True otherwise
	 */
	public boolean canRotate() {
		// Displays cannot be rotated ever since they implement the Stop
		// behavior for inputs.
		return false;
	}

	/**
	 * Tells if a display is capable of flippinging, can only flip when input
	 * has no attachment.
	 * 
	 * @return False if input has a wire attached, True otherwise
	 */
	public boolean canFlip() {
		// Displays cannot be flipped ever since they implement the Stop
		// behavior for inputs.
		return false;
	}
